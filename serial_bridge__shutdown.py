...
def main():
    ...
    WARMUP_MS = int(os.environ.get("WARMUP_MS", "200"))  # e.g., 300
    ...
    # Drop any stale bytes so we start at a clean line boundary
    ...
    try:
        while running:
            ...
            read_fds = [serial_fd] + [mfd for (mfd, _, _) in vpty]
            ...
            if serial_fd in rlist:
                ...
                if chunk:
                    ...
                    # parse for shutdown trigger
                    buf_serial_in.extend(chunk)
                    while True:
                        nl = buf_serial_in.find(b"\n")
                        if nl == -1:
                            break
                        raw = buf_serial_in[:nl]
                        del buf_serial_in[: nl + 1]
                        raw = raw.rstrip(b"\r")
                        try:
                ...
                while True:
                    try:
                        ...
                        if e.errno in (errno.EAGAIN, errno.EWOULDBLOCK):
                            break
                        if e.errno not in (errno.EIO, errno.EPIPE, errno.ENXIO, errno.EBADF):
                    ...
                    if env_flag("ECHO_PTY_TX", False) and data:
